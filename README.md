...
每类别产 `<cat>.list`（clash/surge/egern 文本）、`<cat>.srs`（sing-box 二进制）、`<cat>.singbox.json`（srs 源）；**纯 domain / 纯 ip-cidr** 类别额外产 `<cat>.mrs`（clash 二进制，3-5× 小、加载/匹配更快）；另产 `<cat>.arrs`（[NodePass Anywhere](https://github.com/NodePassProject/Anywhere)，**超 10k 条单文件上限的大类除外**——cn / reject / geoip-cn 由 Anywhere 内置 Country Bypass / ADBlock 覆盖）。
...
| `geoip-cn` | **`.mrs`**（`behavior: ipcidr`） | `.srs` | `.list` | —（超 10k） |
...
`.arrs` 文件是 headless 的（不含动作）——在 App 内对每个规则集指定 DIRECT / REJECT / 某代理。CN 直连与广告拦截用 Anywhere 内置的 Country Bypass / ADBlock，无需订阅 cn / reject / geoip-cn。
...
├── anywhere/{netflix,ai,gfw,...}.arrs   # NodePass Anywhere；超 10k 上限大类（cn/reject/geoip-cn）跳过
