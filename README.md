...
每类别产 `<cat>.list`（clash/surge/egern 文本）、`<cat>.srs`（sing-box 二进制）、`<cat>.singbox.json`（srs 源）；**纯 domain / 纯 ip-cidr** 类别额外产 `<cat>.mrs`（clash 二进制，3-5× 小、加载/匹配更快）；另产 `<cat>.arrs`（[NodePass Anywhere](https://github.com/NodePassProject/Anywhere)，**超 10k 条单文件上限的大类除外**——实测 cn / reject 跳过，由 Anywhere 内置 Country Bypass / ADBlock 覆盖）。

> 上述都是 **per-category** 文件。此外**全类目合一**产**单个** `geosite.dat`（xray/v2ray 原生格式，按 tag 索引；消费端 `geosite:<tag>` 引用，见下方 xray/v2ray 段）。

| 类别 | clash | sing-box | surge / sr / egern | anywhere | xray / v2ray |
|------|-------|----------|--------------------|----------|--------------|
| `cn` `reject` `gfw` `youtube` `github` `cloudflare` `telegram` `twitter` | **`.mrs`**（`behavior: domain`） | `.srs` | `.list` | `.arrs`（cn/reject 超 10k 不产） | `geosite:<tag>` |
...
### anywhere（NodePass，`.arrs`）

App 内 **Routing Rules → 添加规则集订阅**，填 `.arrs` URL（路径以 `.arrs` 结尾即可被识别）：

```text
https://cdn.jsdelivr.net/gh/river2heaven/tongtian@ruleset-dist/dist/gfw.arrs
https://cdn.jsdelivr.net/gh/river2heaven/tongtian@ruleset-dist/dist/netflix.arrs
https://cdn.jsdelivr.net/gh/river2heaven/tongtian@ruleset-dist/dist/ai.arrs
```

`.arrs` 文件是 headless 的（不含动作）——在 App 内对每个规则集指定 DIRECT / REJECT / 某代理。CN 直连与广告拦截建议用 Anywhere 内置的 Country Bypass / ADBlock（cn / reject 超 10k 上限无 `.arrs`；`geoip-cn.arrs` 有产出，需要 IP 级 CN 直连时可订阅）。

### xray / v2ray（`geosite.dat`）

xray/v2ray 用按 tag 索引的 `geosite.dat` domain 数据库，路由里以 `geosite:<tag>` 匹配。本编译器全类目合一产**单个** `geosite.dat`（domain 侧规则全进，IP 侧不进——IP 用各发行版自带的 `geoip.dat`），tag 即类目名（大小写不敏感）：`geosite:ai` / `geosite:netflix` / `geosite:gfw` …

1. 下载 `geosite.dat` 放到 xray/v2ray 的 asset 目录（默认 `/usr/local/share/xray/`，或用 `XRAY_LOCATION_ASSET` / `V2RAY_LOCATION_ASSET` 环境变量指定）：

```bash
curl -fsSL -o /usr/local/share/xray/geosite.dat \
  "https://cdn.jsdelivr.net/gh/river2heaven/tongtian@ruleset-dist/dist/geosite.dat"
```

2. 在配置的 `routing.rules` 里引用 tag（动作即 `outboundTag`，由你自己指定）：

```jsonc
"routing": {
  "rules": [
    { "type": "field", "domain": ["geosite:netflix"], "outboundTag": "streaming" },
    { "type": "field", "domain": ["geosite:ai"],      "outboundTag": "proxy" },
    { "type": "field", "domain": ["geosite:cn"],      "outboundTag": "direct" }
  ]
}
```

> 与上面 per-category 远程订阅不同，`geosite.dat` 是**本地 asset**（xray/v2ray 不支持远程 rule-provider）——更新即重新下载该文件（生产建议钉 `@<tag>` URL，变更可控可回退）。

> **非致命投递**：per-category 订阅（clash/singbox/surge/anywhere）一律远程拉取、失败即降级不致命；绝不用会致命 bootstrap 的整包 geodata。生产建议钉 `@<tag>` 而非滚动 `@ruleset-dist`，变更可控可回退（详见 §5）。
...
| xray / v2ray | `geosite.dat`（全类目合一单文件，本地 asset） | 纯 Go（直拼 protobuf，零外部 CLI） |
| anywhere ([NodePass](https://github.com/NodePassProject/Anywhere)) | `.arrs`（纯文本，headless；单文件硬上限 10k 条，超限类别跳过产出） | 文本模板 |

### 4.3 能力天花板（按目标降级）

统一格式能表达的 = **各客户端能力的交集**；超出的按目标降级（支持的正常编、不支持的跳过 + 编译告警）。

| matcher | clash | sing-box | surge/sr | anywhere | 说明 |
|---------|:---:|:---:|:---:|:---:|------|
| DOMAIN / -SUFFIX / -KEYWORD | ✅ | ✅ | ✅ | ⚠️ | 交集；anywhere 无 exact 类型，DOMAIN 降级 suffix |
| IP-CIDR / IP-CIDR6 | ✅ | ✅ | ✅ | ✅ | 交集 |
...
├── anywhere/{netflix,ai,gfw,geoip-cn,...}.arrs   # NodePass Anywhere；超 10k 上限大类（cn/reject）跳过
├── geoip/cn.{list,mrs,srs}          # 来自 chnroutes2
├── geosite.dat                      # xray/v2ray 原生：全类目合一单文件（按 tag 索引）
