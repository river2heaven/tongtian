...
| 类别 | clash | sing-box | surge / sr / egern | anywhere |
|------|-------|----------|--------------------|----------|
| `cn` `reject` `gfw` `youtube` `github` `cloudflare` `telegram` `twitter` | **`.mrs`**（`behavior: domain`） | `.srs` | `.list` | `.arrs`（cn/reject 超 10k 不产） |
| `geoip-cn` | **`.mrs`**（`behavior: ipcidr`） | `.srs` | `.list` | `.arrs`（~4k CIDR） |
| `netflix` `ai` `disney`（含 regex/keyword，mrs 不支持） | `.list`（`behavior: classical`） | `.srs` | `.list` | `.arrs`（regex 行丢弃） |
